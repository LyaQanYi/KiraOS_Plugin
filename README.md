...
依赖（KiraAI 主程序通常已带）：`jieba`（中文分词）、`tomli_w`（TOML 写入）。Python 3.10 还需 `tomli`，3.11+ 走标准库 `tomllib`。
...
### 启动报 `tomli_w not found`
v3 用 TOML 作真相源。安装：`pip install tomli_w` （Python 3.10 还需 `pip install tomli`，3.11+ 走标准库 `tomllib`）

### 启动报 `jieba not found`
`memory_index.py` 用 jieba 做中文分词驱动 FTS5。`pip install jieba`
